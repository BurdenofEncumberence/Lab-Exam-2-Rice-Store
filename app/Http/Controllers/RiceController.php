...
class RiceController extends Controller
{
    ...
    {
        $validated = $request->validate(['name' => 'required|string|max:255', 'price' => 'required|numeric|min:0', 'stockAmount' => 'required|integer|min:0',
        'description' => 'nullable|string',]);

        Rice::create($validated);

        return redirect ()->route('rices.index')->with('success', 'Rice product was added successfully!');
    }
    ...
    public function update(Request $request, Rice $rice)
    {
        $validated = $request->validate([
            ...
            'stockAmount' => 'required|integer|min:0',
            'description' => 'nullable|string',
        ]);
